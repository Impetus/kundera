...
/**
 * Parser for handling JPQL Single-String queries. Takes a JPQLQuery and the query string and parses it into its
 * constituent parts, updating the JPQLQuery accordingly with the result that after calling the parse() method the
...
public class KunderaQueryParser {
    ...
    /**
     * Compiler to process keywords contents. In the query the keywords often have content values following them that
     * represent the constituent parts of the query. This takes the keyword and sets the constituent part accordingly.
     */
        ...
        private void compileUpdateClause() {
            ...
            if (query.isUpdateStatement() && query.getUpdateStatement().getUpdateClause().hasUpdateItems()) {
                ...
                if (query.getUpdateStatement().getUpdateClause().getUpdateItems() instanceof CollectionExpression) {
                    ...
                    updateColumnIter =
                        ((CollectionExpression) query.getUpdateStatement().getUpdateClause().getUpdateItems())
                            .children().iterator();
                    ...
                    updateColumnIter =
                        query.getUpdateStatement().getUpdateClause().getUpdateItems().children().iterator();
    ...
    private void buildResultColumns() {
        if (query.isSelectStatement()) {
            ...
            query.setAggregated(aggregationCount > 0 ? true : false);
            result = new String[size + 1 - aggregationCount];
            aggResult = new String[aggregationCount + 1];
            int count = 0, aggCounter = 0;
            ...
            if (selectColumnIter != null) {
                while (selectColumnIter.hasNext()) {
                    ...
                    if (validateExpression(nextExpression)) {
                        aggCounter =
                            buildResult(aggResult, aggCounter,
