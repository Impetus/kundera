...
public class KunderaIndexer implements Indexer {
	...
	private static final String SUPERCOLUMN_INDEX = UUID
			+ ".entity.super.indexname";
	...
	private void performIndexing(EntityMetadata metadata, Object object) {
		...
		if (metadata.getType().equals(EntityMetadata.Type.SUPER_COLUMN_FAMILY)) {
			Map<String, EmbeddedColumn> superColMap = metadata
				...
				try {
					...
					if (embeddedObject instanceof Collection<?>) {
						for (Object obj : (Collection<?>) embeddedObject) {
							currentDoc = prepareDocument(metadata, object,
									superColumnName);
	 ...
	 * @return the document
	 */
	private Document prepareDocument(EntityMetadata metadata, Object object,
	 ...
	 *            the super column
	 */
	private void indexSuperColumn(EntityMetadata metadata, Object object,
			Document currentDoc, Object embeddedObject,
	 ...
	 *            the current doc
	 */
	private void indexSuperColumnName(String superColumnName,
			Document currentDoc) {
		Field luceneField = new Field(SUPERCOLUMN_INDEX, superColumnName,
	...
	private void indexDocument(EntityMetadata metadata, Document document) {
		try {
			...
			if (metadata.getDBType().equals(DBType.CASSANDRA)) {
				...
				LOG.debug("Indexing document in file system using lucene: "
						+ document);
	 ...
	 *            the document
	 */
	private void addIndexProperties(EntityMetadata metadata, Object object,
			Document document) {
		String indexName = metadata.getIndexName();
	 ...
	 *            the document
	 */
	private void prepareIndexDocument(EntityMetadata metadata, Object object,
			Document document) {
		try {
			...
			luceneField = new Field(ENTITY_CLASS_FIELD, metadata
					.getEntityClazz().getCanonicalName().toLowerCase(),
					Field.Store.YES, Field.Index.ANALYZED_NO_NORMS);
			document.add(luceneField);

			// index index name
			luceneField = new Field(ENTITY_INDEXNAME_FIELD,
					metadata.getIndexName(), Field.Store.YES,
	...
	private void indexDocumentUsingLucandra(Document document) {
		try {
			...
			indexWriter.addDocument(INDEX_NAME, document, analyzer,
					docNumber++, true, rms);
		} catch (CorruptIndexException e) {
			throw new IndexingException(e.getMessage());
		} catch (IOException e) {
	...
	private void indexField(Object object, Document document,
		...
		try {
			...
			if (value != null) {
				Field luceneField = new Field(getCannonicalPropertyName(
						indexName, colName), value, Field.Store.NO,
	...
	@Override
	public final Map<String, String> search(String luceneQuery, int start,
			int count) {
		...
		LOG.debug("Searching index with query[" + luceneQuery + "], start:"
				+ start + ", count:" + count);
