 ...
 * @author amresh.singh
 */
public abstract class CassandraClientBase extends ClientBase implements
		ClientPropertiesSetter {

	/** log for this class. */
	private static Logger log = LoggerFactory
			.getLogger(CassandraClientBase.class);
	 ...
	 */
	protected Column populateFkey(String rlName, Object rlValue, long timestamp)
			throws PropertyAccessException {
	 ...
	 * @return the list
	 */
	protected List<Object> onCounterColumn(EntityMetadata m,
			boolean isRelation, List<String> relations, List<KeySlice> ks) {
		List<Object> entities;

		if (m.getType().isSuperColumnFamilyMetadata()) {
			if (log.isInfoEnabled()) {
				log.info(
						"On counter column for super column family of entity {}.",
						m.getEntityClazz());
			}
			...
			for (KeySlice slice : ks) {
				counterColumns = new ArrayList<CounterSuperColumn>(
						slice.getColumnsSize());
			...
			for (byte[] key : results.keySet()) {
				Object e = null;
				Object id = PropertyAccessorHelper.getObject(m.getIdAttribute()
						.getJavaType(), key);
				List<CounterSuperColumn> counterSuperColumns = results.get(key);
				ThriftRow tr = new ThriftRow(id, m.getTableName(),
						new ArrayList<Column>(0),
						new ArrayList<SuperColumn>(0),
						new ArrayList<CounterColumn>(0), counterSuperColumns);
				e = getDataHandler().populateEntity(tr, m,
						CassandraUtilities.getEntity(e), relations, isRelation);
				entities.add(e);
			}
			...
			if (log.isInfoEnabled()) {
				log.info("On counter column for column family of entity {}",
						m.getEntityClazz());
			}
			...
			for (byte[] key : results.keySet()) {
				...
				ThriftRow tr = new ThriftRow(id, m.getTableName(),
						new ArrayList<Column>(0),
						new ArrayList<SuperColumn>(0), columns,
	 ...
	 *            the q results
	 */
	protected void computeEntityViaColumns(EntityMetadata m,
			boolean isRelation, List<String> relations, List<Object> entities,
			Map<ByteBuffer, List<Column>> qResults) {
		...
		List<AbstractManagedType> subManagedType = ((AbstractManagedType) entityType)
				.getSubManagedType();

		for (ByteBuffer key : qResults.keySet()) {
			onColumn(m, isRelation, relations, entities, qResults.get(key),
					subManagedType, key);
		}
	}

	protected void onColumn(EntityMetadata m, boolean isRelation,
			List<String> relations, List<Object> entities,
			List<Column> columns, List<AbstractManagedType> subManagedType,
			ByteBuffer key) {
		if (!columns.isEmpty()) {
			Object id = PropertyAccessorHelper.getObject(m.getIdAttribute()
					.getJavaType(), key.array());
			ThriftRow tr = new ThriftRow(id, m.getTableName(), columns,
					new ArrayList<SuperColumn>(0),
					new ArrayList<CounterColumn>(0),
					new ArrayList<CounterSuperColumn>(0));
			Object o = null;
			...
			if (!subManagedType.isEmpty()) {
				for (AbstractManagedType subEntity : subManagedType) {
					EntityMetadata subEntityMetadata = KunderaMetadataManager
							.getEntityMetadata(kunderaMetadata,
									subEntity.getJavaType());

					o = getDataHandler().populateEntity(tr, subEntityMetadata,
							CassandraUtilities.getEntity(o),
				...
				o = getDataHandler().populateEntity(tr, m,
						CassandraUtilities.getEntity(o), relations, isRelation);
			}

			if (log.isInfoEnabled()) {
				log.info(
						"Populating data for entity of clazz {} and row key {}.",
						m.getEntityClazz(), tr.getId());
			}
	 ...
	 *            the q results
	 */
	protected void computeEntityViaSuperColumns(EntityMetadata m,
			boolean isRelation, List<String> relations, List<Object> entities,
			Map<ByteBuffer, List<SuperColumn>> qResults) {
		for (ByteBuffer key : qResults.keySet()) {
			onSuperColumn(m, isRelation, relations, entities,
					qResults.get(key), key);
		}
	}

	protected void onSuperColumn(EntityMetadata m, boolean isRelation,
			List<String> relations, List<Object> entities,
		...
		ThriftRow tr = new ThriftRow(id, m.getTableName(),
				new ArrayList<Column>(0), superColumns,
				new ArrayList<CounterColumn>(0),
				new ArrayList<CounterSuperColumn>(0));
		...
		if (log.isInfoEnabled()) {
			log.info(
					"Populating data for super column family of clazz {} and row key {}.",
					m.getEntityClazz(), tr.getId());
		}
	 ...
	 *            the relations
	 */
	protected void addRelationsToThriftRow(EntityMetadata metadata,
			ThriftRow tf, List<RelationHolder> relations) {
		if (relations != null) {
			...
			for (RelationHolder rh : relations) {
				...
				if (linkName != null && linkValue != null) {
					if (metaModel.getEmbeddables(metadata.getEntityClazz())
							.isEmpty()) {
						if (metadata.isCounterColumnType()) {
							CounterColumn col = populateCounterFkey(linkName,
									linkValue);
							tf.addCounterColumn(col);
						} else {
							Column col = populateFkey(linkName, linkValue,
									timestamp);
							tf.addColumn(col);
						}
						...
						if (metadata.isCounterColumnType()) {
							...
							CounterColumn column = populateCounterFkey(
									linkName, linkValue);
							...
							Column column = populateFkey(linkName, linkValue,
									timestamp);
	 ...
	 *            the consistency level
	 */
	protected void deleteRecordFromCounterColumnFamily(Object pKey,
			String tableName, EntityMetadata metadata,
		...
		try {
			...
			if (log.isInfoEnabled()) {
				log.info("Removing data for counter column family {}.",
						tableName);
			}

			conn.remove_counter((CassandraUtilities.toBytes(pKey, metadata
					.getIdAttribute().getJavaType())), path, consistencyLevel);
	 ...
	 * @param columnType
	 */
	protected void createIndexesOnColumns(EntityMetadata m, String tableName,
			List<Column> columns, Class columnType) {
		Object pooledConnection = null;
		try {
			...
			if (columnFamilyDefToUpdate == null) {
				...
				throw new PersistenceException("table" + tableName
						+ " not found!");
			}
			// create a column family, in case it is not already available.

			// Get list of indexes already created
			List<ColumnDef> columnMetadataList = columnFamilyDefToUpdate
					.getColumn_metadata();
			List<String> indexList = new ArrayList<String>();

			if (columnMetadataList != null) {
				for (ColumnDef columnDef : columnMetadataList) {
					indexList.add(new StringAccessor().fromBytes(String.class,
							columnDef.getName()));
				}
			...
			for (Column column : columns) {
				...
				columnDef.setValidation_class(CassandraValidationClassMapper
						.getValidationClass(columnType, false));
				...
				// definition)
				if (!indexList.contains(new StringAccessor().fromBytes(
			...
			// definition
			if (isUpdatable) {
				columnFamilyDefToUpdate
						.setKey_validation_class(CassandraValidationClassMapper
								.getValidationClass(m.getIdAttribute()
										.getJavaType(), isCql3Enabled(m)));
				api.system_update_column_family(columnFamilyDefToUpdate);
			}

		} catch (Exception e) {
			log.warn(
					"Could not create secondary index on column family {}, Caused by: . ",
					tableName, e);
	 ...
	 * @return the list
	 */
	public <E> List<E> findAll(Class<E> entityClass, String[] columnsToSelect,
			Object... rowIds) {
		EntityMetadata entityMetadata = KunderaMetadataManager
				.getEntityMetadata(kunderaMetadata, entityClass);
		List<E> results = new ArrayList<E>();
		results = find(entityClass, entityMetadata.getRelationNames(),
				entityMetadata.getRelationNames() != null
						&& !entityMetadata.getRelationNames().isEmpty(),
	 ...
	 * @return the object
	 */
	private final Object find(Class<?> clazz, EntityMetadata metadata,
			Object rowId, List<String> relationNames) {
		...
		try {
			...
			List<ManagedType> subTypes = ((AbstractManagedType) entityType)
					.getSubManagedType();
			...
			if (!subTypes.isEmpty()) {
				for (ManagedType subEntity : subTypes) {
					EntityMetadata subEntityMetadata = KunderaMetadataManager
							.getEntityMetadata(kunderaMetadata,
									subEntity.getJavaType());
					result = populate(clazz, subEntityMetadata, rowId,
							subEntityMetadata.getRelationNames(), metaModel);
					if (result != null && !result.isEmpty()) {
						break;
					}
				}
			} else {
				result = populate(clazz, metadata, rowId, relationNames,
						metaModel);
			}
		} catch (Exception e) {
			log.error(
					"Error while retrieving records from database for entity {} and key {}, Caused by: .",
					clazz, rowId, e);
	...
	private List<Object> populate(Class<?> clazz, EntityMetadata metadata,
			Object rowId, List<String> relationNames, MetamodelImpl metaModel) {
		List<Object> result;
		if (isCql3Enabled(metadata)) {
			result = cqlClient.find(metaModel, metadata, rowId, relationNames);
		} else {
			result = (List<Object>) find(clazz, relationNames,
					relationNames != null, metadata, rowId);
		}
		return result;
	}
	...
	public boolean isCql3Enabled(EntityMetadata metadata) {
		if (metadata != null) {
			...
			if (metaModel.isEmbeddable(metadata.getIdAttribute()
					.getBindableJavaType())) {
				return true;
			} else if (getCqlVersion().equalsIgnoreCase(
			...
			return getCqlVersion().equalsIgnoreCase(
					CassandraConstants.CQL_VERSION_3_0);
		}
		return getCqlVersion().equalsIgnoreCase(
				CassandraConstants.CQL_VERSION_3_0);
	...
	public <E> List<E> find(Class<E> entityClass,
			Map<String, String> superColumnMap, CassandraDataHandler dataHandler) {
		List<E> entities = null;
		String entityId = null;
		try {
			EntityMetadata entityMetadata = KunderaMetadataManager
					.getEntityMetadata(kunderaMetadata, getPersistenceUnit(),
							entityClass);
			entities = new ArrayList<E>();
			for (String superColumnName : superColumnMap.keySet()) {
				entityId = superColumnMap.get(superColumnName);
				List<SuperColumn> superColumnList = loadSuperColumns(
						entityMetadata.getSchema(),
						entityMetadata.getTableName(), entityId,
						new String[] { superColumnName.substring(0,
								superColumnName.indexOf("|")) });
				E e = (E) dataHandler.fromThriftRow(
						entityMetadata.getEntityClazz(),
						entityMetadata,
						new DataRow<SuperColumn>(entityId, entityMetadata
								.getTableName(), superColumnList));
				if (e != null) {
					entities.add(e);
				}
			}
	 ...
	 * @return the list
	 */
	public List executeSelectQuery(Class clazz, List<String> relationalField,
			CassandraDataHandler dataHandler, boolean isNative, String cqlQuery) {
		if (log.isInfoEnabled()) {
		...
		EntityMetadata entityMetadata = KunderaMetadataManager
				.getEntityMetadata(kunderaMetadata, clazz);
		...
		if (subManagedType.isEmpty()) {
			entities.addAll(cqlClient.executeQuery(clazz, relationalField,
			...
			for (AbstractManagedType subEntity : subManagedType) {
				...
				entities.addAll(cqlClient.executeQuery(
						subEntityMetadata.getEntityClazz(), relationalField,
						dataHandler, true, isNative, cqlQuery));
			}
		}
		return entities;
	 ...
	 *             the exception
	 */
	protected List populateEntitiesFromKeySlices(EntityMetadata m,
			boolean isWrapReq, List<String> relations, List<KeySlice> keys,
			CassandraDataHandler dataHandler) throws Exception {
		List results;
		MetamodelImpl metaModel = (MetamodelImpl) kunderaMetadata
				.getApplicationMetadata().getMetamodel(m.getPersistenceUnit());

		Set<String> superColumnAttribs = metaModel.getEmbeddables(
				m.getEntityClazz()).keySet();
		...
		for (KeySlice key : keys) {
			...
			Object e = null;
			Map<ByteBuffer, List<ColumnOrSuperColumn>> data = new HashMap<ByteBuffer, List<ColumnOrSuperColumn>>(
					1);
			data.put(ByteBuffer.wrap(rowKey), columns);
			ThriftRow tr = new ThriftRow();
			tr.setId(id);
			tr.setColumnFamilyName(m.getTableName());
			tr = dataGenerator.translateToThriftRow(data,
					m.isCounterColumnType(), m.getType(), tr);

			e = dataHandler.populateEntity(tr, m,
					CassandraUtilities.getEntity(e), relations, isWrapReq);
	 ...
	 * @return
	 */
	protected List<String> createInsertQuery(EntityMetadata entityMetadata,
			Object entity, Cassandra.Client cassandra_client,
			List<RelationHolder> rlHolders, Object ttlColumns) {
		List<String> insert_Queries = new ArrayList<String>();
		CQLTranslator translator = new CQLTranslator();
		HashMap<TranslationType, Map<String, StringBuilder>> translation = translator
				.prepareColumnOrColumnValues(entity, entityMetadata,
						TranslationType.ALL, externalProperties,
						kunderaMetadata);

		Map<String, StringBuilder> columnNamesMap = translation
				.get(TranslationType.COLUMN);
		Map<String, StringBuilder> columnValuesMap = translation
		...
		for (String tableName : columnNamesMap.keySet()) {
			...
			insert_Query = StringUtils.replace(insert_Query,
					...
					translator
							.ensureCase(new StringBuilder(), tableName, false)
							.toString());
			...
			for (RelationHolder rl : rlHolders) {
				...
				translator.appendColumnName(columnNameBuilder,
						rl.getRelationName());
				translator.appendValue(columnValueBuilder, rl
						.getRelationValue().getClass(), rl.getRelationValue(),
						true, false);
			}

			insert_Query = StringUtils.replace(insert_Query,
					CQLTranslator.COLUMN_VALUES, columnValueBuilder.toString());
			insert_Query = StringUtils.replace(insert_Query,
					CQLTranslator.COLUMNS, columnNameBuilder.toString());
	 ...
	 * @return
	 */
	protected List<String> createUpdateQueryForCounter(
		...
		for (Attribute attrib : attributes) {
			if (!entityMetadata.getIdAttribute().getName()
					.equals(attrib.getName())
					&& !metaModel.isEmbeddable(attrib.getJavaType())
					&& !attrib.isAssociation()) {
				...
				translator.buildSetClauseForCounters(
						builder,
		...
		for (RelationHolder rl : rlHolders) {
			translator.buildSetClauseForCounters(
					new StringBuilder(builders.get(entityMetadata
							.getTableName())), rl.getRelationName(), rl
							.getRelationValue());
		}
		...
		for (String tableName : builders.keySet()) {
			...
			update_Query = StringUtils.replace(update_Query,
					CQLTranslator.COLUMN_FAMILY,
					translator
							.ensureCase(new StringBuilder(), tableName, false)
							.toString());
			...
			// strip last "," clause.
			builder.delete(builder.lastIndexOf(CQLTranslator.COMMA_STR),
					builder.length());

			builder.append(CQLTranslator.ADD_WHERE_CLAUSE);
			onWhereClause(entityMetadata, rowId, translator, builder,
					metaModel, entityMetadata.getIdAttribute());
	 ...
	 *            the compound key
	 */
	protected String onDeleteQuery(EntityMetadata metadata, String tableName,
			MetamodelImpl metaModel, Object keyObject) {
		CQLTranslator translator = new CQLTranslator();
		String deleteQuery = CQLTranslator.DELETE_QUERY;

		deleteQuery = StringUtils.replace(deleteQuery,
				CQLTranslator.COLUMN_FAMILY,
				translator.ensureCase(new StringBuilder(), tableName, false)
						.toString());

		StringBuilder deleteQueryBuilder = new StringBuilder(deleteQuery);

		deleteQueryBuilder.append(CQLTranslator.ADD_WHERE_CLAUSE);
		onWhereClause(metadata, keyObject, translator, deleteQueryBuilder,
				metaModel, metadata.getIdAttribute());

		// strip last "AND" clause.
		deleteQueryBuilder.delete(
				deleteQueryBuilder.lastIndexOf(CQLTranslator.AND_CLAUSE),
				deleteQueryBuilder.length());

		if (log.isInfoEnabled()) {
			log.info("Returning delete query {}.",
	 ...
	 *            the compound key
	 */
	protected void onWhereClause(EntityMetadata metadata, Object key,
			CQLTranslator translator, StringBuilder queryBuilder,
			MetamodelImpl metaModel, SingularAttribute attribute) {
		// SingularAttribute idAttribute = metadata.getIdAttribute();
		if (metaModel.isEmbeddable(attribute.getBindableJavaType())) {
			Field[] fields = attribute.getBindableJavaType()
					.getDeclaredFields();
			EmbeddableType compoundKey = metaModel.embeddable(attribute
					.getBindableJavaType());
			...
			for (Field field : fields) {
				if (field != null && !Modifier.isStatic(field.getModifiers())
					...
					attribute = (SingularAttribute) compoundKey
							.getAttribute(field.getName());
					Object valueObject = PropertyAccessorHelper.getObject(key,
							field);
					if (metaModel.isEmbeddable(((AbstractAttribute) attribute)
							.getBindableJavaType())) {
						onWhereClause(metadata, valueObject, translator,
								queryBuilder, metaModel, attribute);
					} else {
						String columnName = ((AbstractAttribute) attribute)
								.getJPAColumnName();
						translator.buildWhereClause(queryBuilder,
								field.getType(), columnName, valueObject,
			...
			translator.buildWhereClause(queryBuilder,
					...
					CassandraUtilities.getIdColumnName(kunderaMetadata,
							metadata, getExternalProperties(),
							isCql3Enabled(metadata)), key,
	 ...
	 * @return the list
	 */
	public abstract List find(Class entityClass, List<String> relationNames,
			boolean isWrapReq, EntityMetadata metadata, Object... rowIds);
	 ...
	 * @return the list
	 */
	protected abstract List<SuperColumn> loadSuperColumns(String keyspace,
	 ...
	 * @return the list
	 */
	public abstract List executeQuery(Class clazz,
			List<String> relationalField, boolean isNative, String cqlQuery);
	 ...
	 * @return the list
	 */
	public abstract List find(List<IndexClause> ixClause, EntityMetadata m,
			boolean isRelation, List<String> relations, int maxResult,
			List<String> columns);
	 ...
	 *             the exception
	 */
	public abstract List findByRange(byte[] muinVal, byte[] maxVal,
			EntityMetadata m, boolean isWrapReq, List<String> relations,
			List<String> columns, List<IndexExpression> conditions,
	 ...
	 * @return the list
	 */
	public abstract List<SearchResult> searchInInvertedIndex(
			String columnFamilyName, EntityMetadata m,
	 ...
	 * @return the list
	 */
	public abstract List<EnhanceEntity> find(EntityMetadata m,
			List<String> relationNames, List<IndexClause> conditions,
		...
		try {
				...
				if (node.isDirty()) {
					...
					if (isCql3Enabled(metadata)) {
						...
						if (node.isInState(RemovedState.class)) {
							String query;
							query = onDeleteQuery(metadata,
									metadata.getTableName(), metaModel, id);
							batchQueryBuilder.append(query);
						} else {
							List<String> insertQueries = createInsertQuery(
									metadata, entity, conn, relationHolders,
						...
						if (node.isInState(RemovedState.class)) {
							...
							mutationMap = prepareMutation(metadata, entity, id,
									relationHolders, mutationMap);

							recordsExecuted += mutationMap.size();
							if (!batchMutationMap.containsKey(metadata
									.getEntityClazz())) {
								batchMutationMap.put(metadata.getEntityClazz(),
										mutationMap);
							} else {
								batchMutationMap.get(metadata.getEntityClazz())
										.putAll(mutationMap);
							}
			...
			if (!batchMutationMap.isEmpty()) {
				...
				for (Class<?> entityClass : batchMutationMap.keySet()) {
					conn.batch_mutate(batchMutationMap.get(entityClass),
							consistencyLevel);
				}
			}
	 ...
	 * @return the map
	 */
	protected Map<ByteBuffer, Map<String, List<Mutation>>> prepareMutation(
			EntityMetadata entityMetadata, Object entity, Object id,
		...
		if (isUpdate && entityMetadata.isCounterColumnType()) {
			log.warn(
					"Invalid operation! {} is not possible over counter column of entity {}.",
					"Merge", entityMetadata.getEntityClazz());
		...
		try {
			...
			tfRows = getDataHandler().toThriftRow(entity, id, entityMetadata,
					columnFamily, getTtlValues().get(columnFamily));
		} catch (Exception e) {
			log.error(
					"Error during persisting record for entity {}, Caused by: .",
					entityMetadata.getEntityClazz(),
		...
		for (ThriftRow tf : tfRows) {
			...
			if (entityMetadata.isCounterColumnType()) {
				List<CounterColumn> thriftCounterColumns = tf
						.getCounterColumns();
				List<CounterSuperColumn> thriftCounterSuperColumns = tf
						.getCounterSuperColumns();

				if (thriftCounterColumns != null
						&& !thriftCounterColumns.isEmpty()) {
					for (CounterColumn column : thriftCounterColumns) {
						Mutation mut = new Mutation();
						mut.setColumn_or_supercolumn(new ColumnOrSuperColumn()
				...
				if (thriftCounterSuperColumns != null
						&& !thriftCounterSuperColumns.isEmpty()) {
					for (CounterSuperColumn sc : thriftCounterSuperColumns) {
						Mutation mut = new Mutation();
						mut.setColumn_or_supercolumn(new ColumnOrSuperColumn()
								.setCounter_super_column(sc));
						mutationList.add(mut);
					}
				}
			} else
		...
		ByteBuffer b = CassandraUtilities.toBytes(id, entityMetadata
	 ...
	 */
	@Override
	public void populateClientProperties(Client client,
	...
	public Long getGeneratedValue(TableGeneratorDiscriptor descriptor, String pu) {
		...
		try {
			...
			if (isCql3Enabled()) {
				CQLTranslator translator = new CQLTranslator();
				execute(translator.buildUpdateQuery(descriptor).toString(),
						conn);

				CqlResult result = execute(
						translator.buildSelectQuery(descriptor).toString(),
						conn);

				for (CqlRow row : result.getRows()) {
					latestCount = ByteBufferUtil.toLong(ByteBuffer.wrap(row
				...
				columnPath
						.setColumn(descriptor.getValueColumnName().getBytes());

				try {
					latestCount = conn.get(ByteBuffer.wrap(descriptor
							.getPkColumnValue().getBytes()), columnPath,
							getConsistencyLevel()).counter_column.value;
				} catch (NotFoundException e) {
					log.warn(
							"Counter value not found for {}, resetting it to zero.",
				...
				CounterColumn counterColumn = new CounterColumn(
						ByteBuffer.wrap(descriptor.getValueColumnName()
								.getBytes()), 1);

				conn.add(ByteBuffer.wrap(descriptor.getPkColumnValue()
						.getBytes()), columnParent, counterColumn,
						getConsistencyLevel());
			}
			if (latestCount == 0) {
	...
	protected UUID getAutoGeneratedValue() {
		CqlResult cqlResult = execute(
				"Select now() from system.schema_columns",
				getRawClient("system"));
	...
	protected Object executeCQLQuery(String cqlQuery, boolean isCql3Enabled) {
		...
		try {
			...
			if (log.isErrorEnabled()) {
				log.error("Error during executing query {}, Caused by: {} .",
						cqlQuery, ex);
			}
	 ...
	 * @return
	 */
	protected List<Object> findByRelationQuery(EntityMetadata m,
			String columnName, Object columnValue, Class clazz,
			CassandraDataHandler dataHandler) {
		return cqlClient.findByRelationQuery(m, columnName, columnValue, clazz,
				dataHandler);
	}

	private void setBatchSize(String persistenceUnit,
			Map<String, Object> puProperties) {
		String batch_Size = null;

		PersistenceUnitMetadata puMetadata = KunderaMetadataManager
				.getPersistenceUnitMetadata(kunderaMetadata, persistenceUnit);
		...
		batch_Size = (String) (externalBatchSize != null ? externalBatchSize
				: intbatch != null ? intbatch.toString() : null);
		...
		if (!StringUtils.isBlank(batch_Size)) {
			...
			if (batchSize == 0) {
				throw new IllegalArgumentException(
						"kundera.batch.size property must be numeric and > 0.");
			}
		}
	}
	...
	private void populateCqlVersion(Map<String, Object> externalProperties) {
		...
		if (cqlVersion == null
				|| !(cqlVersion != null && (cqlVersion
						.equals(CassandraConstants.CQL_VERSION_2_0) || cqlVersion
						.equals(CassandraConstants.CQL_VERSION_3_0)))) {
			cqlVersion = (CassandraPropertyReader.csmd != null ? CassandraPropertyReader.csmd
					.getCqlVersion() : CassandraConstants.CQL_VERSION_2_0);
		}
	...
	protected class CQLClient {
		...
		public void persist(EntityMetadata entityMetadata, Object entity,
			...
			List<String> queries;
			if (entityMetadata.isCounterColumnType()) {
				queries = createUpdateQueryForCounter(entityMetadata, entity,
						conn, rlHolders);
			} else {
				queries = createInsertQuery(entityMetadata, entity, conn,
						rlHolders, ttlColumns);
			}
		 ...
		 * @return
		 */
		public List executeQuery(Class clazz, List<String> relationalField,
			...
			try {
				...
				if (result != null
						&& (result.getRows() != null || result.getRowsSize() > 0)) {
					returnedEntities = new ArrayList<Object>(
							result.getRowsSize());
					...
					while (iter.hasNext()) {
						...
						thriftRow = new ThriftRow(rowKey,
						...
						e = dataHandler.populateEntity(thriftRow,
								entityMetadata,
								CassandraUtilities.getEntity(e),
								relationalField, relationalField != null
										&& !relationalField.isEmpty());

						e = populateSecondaryTableData(relationalField,
								dataHandler, isCql3Enabled, entityMetadata, e);
		 ...
		 * @return
		 */
		private Object populateSecondaryTableData(List<String> relationalField,
				CassandraDataHandler dataHandler, boolean isCql3Enabled,
				EntityMetadata entityMetadata, Object e) {
			CqlResult result;
			...
			if (!metaModel.isEmbeddable(entityMetadata.getIdAttribute()
				...
				List<String> secondaryTables = ((DefaultEntityAnnotationProcessor) managedType
				...
				for (String tableName : secondaryTables) {
					...
					translator.buildWhereClause(queryBuilder,
							((AbstractAttribute) entityMetadata
									.getIdAttribute()).getBindableJavaType(),
							CassandraUtilities.getIdColumnName(kunderaMetadata,
									entityMetadata, getExternalProperties(),
									isCql3Enabled(entityMetadata)),
							PropertyAccessorHelper.getId(e, entityMetadata),
							translator.EQ_CLAUSE, false);
					// strip last "AND" clause.
					queryBuilder.delete(
							queryBuilder.lastIndexOf(CQLTranslator.AND_CLAUSE),
							queryBuilder.length());

					// Executing.
					result = (CqlResult) executeCQLQuery(
							queryBuilder.toString(), isCql3Enabled);

					if (result != null
							&& (result.getRows() != null || result
									.getRowsSize() > 0)) {
						...
						while (iterator.hasNext()) {
							...
							e = dataHandler.populateEntity(tr, entityMetadata,
									CassandraUtilities.getEntity(e),
									relationalField, relationalField != null
											&& !relationalField.isEmpty());
							break;
		 ...
		 * @return
		 */
		public List<Object> find(MetamodelImpl metaModel,
				EntityMetadata metadata, Object rowId,
			...
			builder.delete(builder.lastIndexOf(CQLTranslator.AND_CLAUSE),
					builder.length());
			return CassandraClientBase.this.executeQuery(
		...
		protected List<Object> findByRelationQuery(EntityMetadata m,
			...
			selectQuery = StringUtils.replace(
					selectQuery,
					CQLTranslator.COLUMN_FAMILY,
					translator.ensureCase(new StringBuilder(),
							m.getTableName(), false).toString());

			StringBuilder selectQueryBuilder = new StringBuilder(selectQuery);
			selectQueryBuilder.append(CQLTranslator.ADD_WHERE_CLAUSE);

			translator.buildWhereClause(selectQueryBuilder,
					columnValue.getClass(), columnName, columnValue,
					CQLTranslator.EQ_CLAUSE, false);
			selectQueryBuilder.delete(
					selectQueryBuilder.lastIndexOf(CQLTranslator.AND_CLAUSE),
					selectQueryBuilder.length());
			return this.executeQuery(clazz, m.getRelationNames(), dataHandler,
					true, false, selectQueryBuilder.toString());
		}
	}
	 ...
	 * Finds a {@link List} of entities from database
	 */
	public final List findByRowKeys(Class entityClass,
			List<String> relationNames, boolean isWrapReq,
			...
			if (!subManagedType.isEmpty()) {
				for (AbstractManagedType subEntity : subManagedType) {
					EntityMetadata subEntityMetadata = KunderaMetadataManager
							.getEntityMetadata(kunderaMetadata,
									subEntity.getJavaType());
					entities = getDataHandler().fromThriftRow(entityClass,
							subEntityMetadata,
				...
				entities = getDataHandler().fromThriftRow(entityClass,
						metadata, relationNames, isWrapReq,
						getConsistencyLevel(), rowIds);
			}
		} catch (Exception e) {
			log.error(
					"Error while retrieving records for entity {}, row keys {}",
					entityClass, rowIds);
			throw new KunderaException(e);
		}
		return entities;
	...
	protected <T> T execute(final String query, Object connection) {
		try {
			...
			return (T) conn.execute_cql3_query(
					ByteBuffer.wrap(query.getBytes(Constants.CHARSET_UTF8)),
	...
	protected void persistJoinTableByCql(JoinTableData joinTableData,
		...
		Map<Object, Set<Object>> joinTableRecords = joinTableData
				.getJoinTableRecords();
		...
		builder.append(translator.ensureCase(new StringBuilder(),
				joinTableData.getJoinColumnName(), false));
		builder.append(CQLTranslator.COMMA_STR);
		builder.append(translator.ensureCase(new StringBuilder(),
				joinTableData.getInverseJoinColumnName(), false));

		insert_Query = StringUtils.replace(insert_Query,
				CQLTranslator.COLUMN_FAMILY,
				translator
						.ensureCase(new StringBuilder(), joinTableName, false)
						.toString());
		...
		for (Object key : joinTableRecords.keySet()) {
			PropertyAccessor accessor = PropertyAccessorFactory
					.getPropertyAccessor((Field) entityMetadata
			...
			for (Object value : values) {
				if (value != null) {
					...
					columnValueBuilder.append(PropertyAccessorHelper
							.getString(key)
							+ "\001"
							+ PropertyAccessorHelper.getString(value));
					columnValueBuilder.append(CQLTranslator.QUOTE_STR);
					columnValueBuilder.append(CQLTranslator.COMMA_STR);
					translator.appendValue(columnValueBuilder, key.getClass(),
							key, true, false);
					columnValueBuilder.append(CQLTranslator.COMMA_STR);
					translator.appendValue(columnValueBuilder,
							value.getClass(), value, true, false);

					insertQuery = StringUtils.replace(insertQuery,
							CQLTranslator.COLUMN_VALUES,
		...
		if (!StringUtils.isBlank(statements.toString())) {
			batch_Query = StringUtils.replace(batch_Query,
					CQLTranslator.STATEMENT, statements.toString());
	 ...
	 * @return
	 */
	protected <E> List<E> getColumnsByIdUsingCql(String schemaName,
			String tableName, String pKeyColumnName, String columnName,
		...
		selectQuery = StringUtils.replace(selectQuery, CQLTranslator.COLUMNS,
				translator.ensureCase(new StringBuilder(), columnName, false)
						.toString());
		...
		translator
				.buildWhereClause(selectQueryBuilder, columnJavaType,
						pKeyColumnName, pKeyColumnValue,
						CQLTranslator.EQ_CLAUSE, false);
		selectQueryBuilder.delete(
				selectQueryBuilder.lastIndexOf(CQLTranslator.AND_CLAUSE),
				selectQueryBuilder.length());

		CqlResult cqlResult = execute(selectQueryBuilder.toString(),
				getRawClient(schemaName));
		...
		while (rowIter.hasNext()) {
			...
			if (!row.getColumns().isEmpty()) {
				...
				Object columnValue = CassandraDataTranslator.decompose(
						columnJavaType, column.getValue(), true);
	 ...
	 * @return
	 */
	protected <E> List<E> findIdsByColumnUsingCql(String schemaName,
			String tableName, String pKeyName, String columnName,
			Object columnValue, Class entityClazz) {
		EntityMetadata metadata = KunderaMetadataManager.getEntityMetadata(
				kunderaMetadata, entityClazz);

		return getColumnsByIdUsingCql(schemaName, tableName, columnName,
				((AbstractAttribute) metadata.getIdAttribute())
						.getJPAColumnName(), columnValue, metadata
						.getIdAttribute().getBindableJavaType());
	}
}
