...
public class Configuration {
	...
	public EntityManager getEntityManager(String persistenceUnit) {
		...
		if (emfMap.get(persistenceUnit) == null) {
			emf = (EntityManagerFactoryImpl) Persistence
		...
		try {
			Map propMap = (Map) PropertyAccessorHelper.getObject(emf, emf
					.getClass().getDeclaredField("props"));
			...
			setField(
					emf,
					emf.getClass().getDeclaredField("cacheProvider"),
					initSecondLevelCache(
							props.getProperty("kundera.cache.provider_class"),
							resourceName));
			...
			logger.info("Kundera Client for Persistence Unit "
					+ persistenceUnit + " is: " + client);
	...
	public void setClient(EntityManager em, String persistenceUnit) {
		try {
			setField(em, em.getClass().getDeclaredField("client"),
					getClient(persistenceUnit));
		} catch (NoSuchFieldException e) {
			throw new PersistenceException(e.getMessage());
		}
	}
	...
	private void createIdentifier(ClientType clientType, String persistenceUnit) {
		identifier = new ClientIdentifier(new String[] { node },
				Integer.valueOf(port), keyspace, clientType, persistenceUnit);
	}
	...
	private CacheProvider initSecondLevelCache(String cacheProviderClassName,
