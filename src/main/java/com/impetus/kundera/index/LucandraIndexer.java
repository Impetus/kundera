...
public class LucandraIndexer implements Indexer {
    ...
    public final void index(EntityMetadata metadata, Object object) {
        ...
        try {
            ...
            // index entity class
            luceneField = new Field(ENTITY_CLASS_FIELD, metadata.getEntityClazz().getCanonicalName(), Field.Store.YES, Field.Index.NOT_ANALYZED_NO_NORMS);
            document.add(luceneField);

            // index index name
            luceneField = new Field(ENTITY_INDEXNAME_FIELD, metadata.getIndexName(), Field.Store.YES, Field.Index.NOT_ANALYZED_NO_NORMS);
            document.add(luceneField);
