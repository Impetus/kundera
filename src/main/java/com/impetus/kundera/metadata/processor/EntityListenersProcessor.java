 ...
 * JPA Specifications:
 * 1. EntityListeners classes must have a no-argument constructor.
 * 2. Callback methods can have any visibility.
 * 3. Callback methods must return void.
 * 4. Callback methods must NOT throw any checked exception.
 * 5. ExternalCallback methods must accept only entity object.
 * 6. InternalCallback methods must NOT accept any parameter.
 * 7. EntityListeners are state-less.
 * 8. EnternalCallbackMethods must be fired before InternalCallbackMethods.
 * 
...
public class EntityListenersProcessor implements MetadataProcessor {
	...
	private static final List<Class<? extends Annotation>> JPAListenersAnnotations = Arrays.asList(PrePersist.class, PostPersist.class, PreUpdate.class,
			PostUpdate.class, PreRemove.class, PostRemove.class, PostLoad.class);
	...
	public final void process(final Class<?> entityClass, EntityMetadata metadata) {
			...
			if (entityListenerClasses != null) {
				...
				for (Class<?> entityListener : entityListenerClasses) {
					...
					try {
						...
						throw new PersistenceException("Skipped method(" + entityListener.getName() + ") must have a default no-argument constructor.");
					}
		...
		// since this is already an @Entity class, it will sure have a default no-arg constructor
