...
public class TableProcessor extends AbstractEntityFieldProcessor {
	...
	private void populateMetadata(EntityMetadata metadata, Class<?> clazz) {
		...
		for (Field f : clazz.getDeclaredFields()) {
			...
			} else if (f.isAnnotationPresent(Embedded.class)) {	//Whether @Embedded	(only for Cassandra)				
	...
	private void populateSuperColumnInMetadata(EntityMetadata metadata, Field superColumnField, Class superColumnClass) {		
		...
		if(ann == null) {
			LOG.warn(superColumnClass + " will not be persisted as Super Column because you didn't annotate it with @Embeddable");
			return;
		}
		
		//TODO: Provide user an option to specify this in entity class rather than default field name
