 ...
 */
public class CassandraClientFactory extends GenericClientFactory
{
    /** The logger. */
    ...
    {          
        //No Cassandra Specific object to set
    }
...
//        Pelops.removePool(PelopsUtils.generatePoolName(getPersistenceUnit(), externalProperties));
    ...
    public SchemaManager getSchemaManager(Map<String, Object> externalProperty)
    {
        if (schemaManager == null)
        {
            initializePropertyReader();
            setExternalProperties(externalProperty);
            schemaManager = new CassandraSchemaManager(CassandraClientFactory.class.getName(), externalProperty);
        }
