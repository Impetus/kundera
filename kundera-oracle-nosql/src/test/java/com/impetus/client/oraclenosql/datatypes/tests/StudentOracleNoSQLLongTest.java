...
public class StudentOracleNoSQLLongTest extends OracleNoSQLBase
{
    ...
    private void findByRange()
    {
        ...
        List<StudentOracleNoSQLLong> students;
        em = emf.createEntityManager();
        query = "Select s From StudentOracleNoSQLLong s where s.id between ?1 and ?2";
        q = em.createQuery(query);
    ...
    private void findByNameAndAgeWithOrClause()
    {
        ...
        int count;
        em = emf.createEntityManager();
        query = "Select s From StudentOracleNoSQLLong s where s.name = Kuldeep and s.age > " + getMinValue(short.class);
        q = em.createQuery(query);
        students = q.getResultList();
        Assert.assertNotNull(students);
        Assert.assertEquals(1, students.size());
