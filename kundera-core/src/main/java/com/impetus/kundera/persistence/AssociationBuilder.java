...
public final class AssociationBuilder {
	...
	void populateRelationForM2M(Object entity, EntityMetadata entityMetadata,
		...
		if (relation.getPropertyType().isAssignableFrom(List.class)
			...
			if (relation.isRelatedViaJoinTable()
					&& (relObject == null
							|| ProxyHelper.isProxyOrCollection(relObject))) {
				populateCollectionFromJoinTable(entity, entityMetadata,
						delegator, relation);
			}
			...
			if (relation.isRelatedViaJoinTable()) {
				...
				populateCollectionFromMap(entity, delegator, relation,
						relObject, relationsMap);
			}
		}
	...
	void populateRelationFromValue(Object entity, PersistenceDelegator pd,
			Relation relation, Object relationValue,
		...
		if (child != null) {
			...
			Field biDirectionalField = getBiDirectionalField(entity.getClass(),
					relation.getTargetEntity());
			boolean isBidirectionalRelation = (biDirectionalField != null);

			if (isBidirectionalRelation) {
				Relation reverseRelation = childMetadata
						.getRelation(biDirectionalField.getName());

				if (relation.getType().equals(ForeignKey.ONE_TO_ONE)) {
					PropertyAccessorHelper.set(child,
							reverseRelation.getProperty(), entity);
				} else {
					Object childId = PropertyAccessorHelper.getId(child,
							childMetadata);
					EntityMetadata reverseEntityMetadata = KunderaMetadataManager
							.getEntityMetadata(entity.getClass());
					populateRelationViaQuery(child, pd, childId,
							reverseRelation, relation.getJoinColumnName(),
	...
	void populateRelationViaQuery(Object entity, PersistenceDelegator pd,
			Object entityId, Relation relation, String relationName,
			EntityMetadata childMetadata) {
		Class<?> childClass = relation.getTargetEntity();
		...
		if (MetadataUtils.useSecondryIndex(childClient.getPersistenceUnit())) {
			...
			associatedObjects = getAssociatedEntitiesFromIndex(entity,
					entityId, childClass, childClient);
		}
		...
		if (isBidirectionalRelation && associatedEntities != null) {
			...
			traversalRequired = (reverseRelation.getType()
					.equals(ForeignKey.ONE_TO_ONE) /*
													 * && ! StringUtils .
													 * isBlank ( reverseRelation
													 * . getMappedBy ( ) )
													 */)
		...
		if (associatedEntities != null) {
			...
			MainCache mainCache = (MainCache) pd.getPersistenceCache()
					.getMainCache();
			...
			for (Object child : associatedEntities) {
				...
				String nodeId = ObjectGraphUtils.getNodeId(childId,
						childMetadata.getEntityClazz());
				Node node = new Node(nodeId, childMetadata.getEntityClazz(),
						new ManagedState(), pd.getPersistenceCache(), childId);
				node.setData(child);
		...
		if ((childMetadata.getRelationNames() == null || childMetadata
			...
			for (Object associatedEntity : associatedEntities) {
				associatedEntity = childClient.getReader()
						.recursivelyFindEntities(associatedEntity, null,
								childMetadata, pd);
			}
		}
	...
	private void populateCollectionFromJoinTable(Object entity,
		...
		EntityMetadata owningEntityMetadata = KunderaMetadataManager
				.getEntityMetadata(relation.getTargetEntity());
		Class columnJavaType = owningEntityMetadata.getIdAttribute()
				.getJavaType();
		if (jtMetadata == null) {
			columnJavaType = entityMetadata.getIdAttribute().getJavaType();
			jtMetadata = owningEntityMetadata.getRelation(
					relation.getMappedBy()).getJoinTableMetadata();
		...
		List<?> foreignKeys = pClient
				.getColumnsById(schema, joinTableName, joinColumnName,
						inverseJoinColumnName, entityId, columnJavaType);

		List childrenEntities = new ArrayList();
		for (Object foreignKey : foreignKeys) {
			EntityMetadata childMetadata = KunderaMetadataManager
					.getEntityMetadata(relation.getTargetEntity());

			Object child = delegator.find(relation.getTargetEntity(),
					foreignKey);
			Object obj = child instanceof EnhanceEntity && child != null ? ((EnhanceEntity) child)
					.getEntity() : child;
			...
			if (isBidirectionalRelation && obj != null) {
				Object columnValue = PropertyAccessorHelper.getId(obj,
						childMetadata);
				Object[] pKeys = pClient.findIdsByColumn(
						entityMetadata.getSchema(), joinTableName,
				...
				PropertyAccessorHelper.set(obj, biDirectionalField, ObjectUtils
						.getFieldInstance(parents, biDirectionalField));
			}
		...
		try {
			PropertyAccessorHelper
					.set(entity,
							childField,
							PropertyAccessorHelper.isCollection(childField
									.getType()) ? ObjectUtils.getFieldInstance(
									childrenEntities, childField)
									: childrenEntities.get(0));
			PersistenceCacheManager.addEntityToPersistenceCache(entity,
					delegator, entityId);
	...
	private void populateCollectionFromMap(Object entity,
		...
		if (relObject == null && relationsMap != null
			...
			for (String relationName : relationsMap.keySet()) {
				...
				if (relationValue instanceof Map) {
					...
					Client targetEntityClient = delegator
							.getClient(childMetadata); // Client
														// for
														// target
														// entity
					for (Object targetEntityKey : relationValueMap.keySet()) {
						// Find target entity from database
						Object targetEntity = targetEntityClient
								.find(childMetadata.getEntityClazz(),
										targetEntityKey);
						if (targetEntity != null
								&& targetEntity instanceof EnhanceEntity) {
							targetEntity = ((EnhanceEntity) targetEntity)
									.getEntity();
						}

						// Set source and target entities into Map key entity
						Object mapKeyEntity = relationValueMap
								.get(targetEntityKey);
						Class<?> relationshipClass = relation
								.getMapKeyJoinClass();
						for (Field f : relationshipClass.getDeclaredFields()) {
							if (f.getType().equals(entity.getClass())) {
								PropertyAccessorHelper.set(mapKeyEntity, f,
										entity);
		...
		if (relObject != null) {
			for (Object child : ((Map) relObject).values()) {
				if (child != null) {
					Object childId = PropertyAccessorHelper.getId(child,
							childMetadata);
					PersistenceCacheManager.addEntityToPersistenceCache(child,
							delegator, childId);
				}
			}
		}
	 ...
	 */
	private List getAssociatedEntitiesFromIndex(Object entity, Object entityId,
			Class<?> childClass, Client childClient) {

		List associatedEntities;
		IndexManager indexManager = childClient.getIndexManager();

		Map<String, Object> results = indexManager != null ? indexManager
				.search(entity.getClass(), childClass, entityId)
				: new HashMap<String, Object>();
		Set rsSet = results != null ? new HashSet(results.values())
				: new HashSet();

		if (childClass.equals(entity.getClass())) {
			associatedEntities = (List<Object>) childClient.findAll(childClass,
					null, rsSet.toArray(new Object[] {}));
		} else {
			associatedEntities = (List<Object>) childClient.findAll(childClass,
					null, rsSet.toArray(new Object[] {}));
		}
	 ...
	 *            Referenced class.
	 */
	public Field getBiDirectionalField(Class originalClazz,
	 ...
	 */
	private Set<?> setAssociatedEntities(Object entity, Field f,
			List<?> associatedEntities) throws PropertyAccessException {
		...
		if (associatedEntities != null) {
			...
			PropertyAccessorHelper
					.set(entity,
							f,
							PropertyAccessorHelper.isCollection(f.getType()) ? ObjectUtils
									.getFieldInstance(associatedEntities, f)
	 ...
	 * @return
	 */
	private boolean isTraversalRequired(Map<String, Object> relationsMap,
			ForeignKey type) {
		return !(relationsMap == null && (type.equals(ForeignKey.ONE_TO_ONE) || type
				.equals(ForeignKey.MANY_TO_ONE)));
	}
	 ...
	 * @param relation
	 */
	public void setProxyRelationObject(Object entity,
			Map<String, Object> relationsMap, EntityMetadata m,
			PersistenceDelegator pd, Object entityId, Relation relation) {
		String relationName = MetadataUtils.getMappedName(m, relation);
		Object relationValue = relationsMap != null ? relationsMap
				.get(relationName) : null;

		if ((relation.getType().equals(ForeignKey.ONE_TO_ONE) || relation
				.getType().equals(ForeignKey.MANY_TO_ONE))) { // One-To-One or
																// Many-To-One
																// relationship
			...
			if(isBidirectionalRelation && relationValue == null)
			{
				...
				Object owner = null;
				KunderaProxy kp = KunderaMetadata.INSTANCE.getCoreMetadata()
					...
					if (relationValue != null) {
						if (log.isDebugEnabled()) {
							log.debug("Creating proxy for >> "
									+ parentEntityMetadata.getEntityClazz().getName() + "#"
									+ relation.getProperty().getName() + "_"
									+ relationValue);
						}
		...
		if (isTraversalRequired(relationsMap, relation.getType())) {
			...
			if (relation.getType().equals(ForeignKey.MANY_TO_MANY)) {
				...
				PersistenceCacheManager.addEntityToPersistenceCache(entity, pd,
						entityId);
				populateRelationForM2M(entity, m, pd, relation, object,
						relationsMap);
			}

			// Populate other type of relationships
			else if (object == null
					|| ProxyHelper.isProxyOrCollection(object)) {
				Field biDirectionalField = getBiDirectionalField(
						relation.getTargetEntity(), entity.getClass());
				boolean isBidirectionalRelation = (biDirectionalField != null);
				...
				if (isBidirectionalRelation
						...
						&& (relation.getType().equals(ForeignKey.ONE_TO_ONE) || relation
								.getType().equals(ForeignKey.MANY_TO_ONE))) {
					relationName = ((AbstractAttribute) m.getIdAttribute())
							.getJPAColumnName();
				} else {
					relationName = MetadataUtils.getMappedName(m, relation);
				}

				relationValue = relationsMap != null ? relationsMap
						.get(relationName) : null;
				...
				if (relationValue != null) {
					...
					populateRelationFromValue(entity, pd, relation,
							relationValue, childMetadata);
					...
					populateRelationViaQuery(entity, pd, entityId, relation,
							relationName, childMetadata);
				}
			}
	...
	private KunderaProxy getLazyEntity(String entityName,
			Class<?> persistentClass, Method getIdentifierMethod,
			Method setIdentifierMethod, Object id, PersistenceDelegator pd) {
		return KunderaMetadata.INSTANCE
				.getCoreMetadata()
				.getLazyInitializerFactory()
				.getProxy(entityName, persistentClass, getIdentifierMethod,
						setIdentifierMethod, id, pd);
	}
