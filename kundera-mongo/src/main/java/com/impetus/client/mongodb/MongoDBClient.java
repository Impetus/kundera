...
public class MongoDBClient implements Client {
	...
	public void writeData(EntityManagerImpl em, EnhancedEntity e,
		...
		if (entity != null) {
			...
			BasicDBObject updatedDocument = new MongoDBDataHandler()
					.getDocumentFromEntity(em, m, e);
			dbCollection.update(searchQuery, updatedDocument);

		} else {
			log.debug("Inserting data into " + dbName + "." + documentName
					+ " for " + key);
			DBCollection dbCollection = mongoDb.getCollection(documentName);

			BasicDBObject document = new MongoDBDataHandler()
					.getDocumentFromEntity(em, m, e);
	 ...
	 */
	@Override
	public <E> E loadData(EntityManagerImpl em, String key, EntityMetadata m)
			throws Exception {
		...
		Object entity = new MongoDBDataHandler().getEntityFromDocument(em,
				m.getEntityClazz(), m, fetchedDocument);
	...
	public <E> List<E> loadData(EntityManagerImpl em, EntityMetadata m,
		...
		log.debug("Fetching data from " + m.getTableName() + " for Keys "
				+ keys);
	...
	public <E> List<E> loadData(EntityManagerImpl em, EntityMetadata m,
		...
		if (result.indexOf(".") >= 0) {
			...
			entities.addAll(new MongoDBDataHandler().getEmbeddedObjectList(
					dbCollection, m, documentName, query));

		} else {
			log.debug("Fetching data from " + documentName + " for Filter "
					+ filterClauseQueue);

			BasicDBObject mongoQuery = new MongoDBDataHandler()
					.createMongoQuery(m, filterClauseQueue);

			DBCursor cursor = dbCollection.find(mongoQuery);

			while (cursor.hasNext()) {
				DBObject fetchedDocument = cursor.next();
				Object entity = new MongoDBDataHandler().getEntityFromDocument(
						em, clazz, m, fetchedDocument);
				entities.add(entity);
			}
		}
	...
	@Override
	public Map<Bytes, List<org.apache.cassandra.thrift.SuperColumn>> loadEmbeddedObjects(
			String keyspace, String columnFamily, String... keys)
			throws Exception {
		throw new NotImplementedException(
				"Method not applicable for MongoDB. Invalid method call");
	}
	...
	public void delete(String idColumnName, String documentName, String rowId)
		...
		if (cursor.hasNext()) {
			...
			throw new PersistenceException("Can't remove Row# " + rowId
					+ " for " + documentName + " because record doesn't exist.");
		}
	...
	public void connect() {
		if (!isConnected) {
			log.info(">>> Connecting to MONGODB at " + contactNode
					+ " on port " + defaultPort);
			try {
				mongo = new Mongo(contactNode, Integer.parseInt(defaultPort));
				mongoDb = mongo.getDB(dbName);
				isConnected = true;
				log.info("CONNECTED to MONGODB at " + contactNode + " on port "
						+ defaultPort);
			} catch (NumberFormatException e) {
				log.error("Invalid format for MONGODB port, Unale to connect!"
						+ "; Details:" + e.getMessage());
			} catch (UnknownHostException e) {
				log.error("Unable to connect to MONGODB at host " + contactNode
						+ "; Details:" + e.getMessage());
			} catch (MongoException e) {
				log.error("Unable to connect to MONGODB; Details:"
						+ e.getMessage());
			}
		}
	}
	...
	public void shutdown() {
		if (isConnected && mongo != null) {
			log.info("Closing connection to MONGODB at " + contactNode
					+ " on port " + defaultPort);
			mongo.close();
			log.info("Connection to MONGODB at " + contactNode + " on port "
					+ defaultPort + " closed");
		} else {
	 ...
	 *            the order
	 */
	public void createIndex(String collectionName, List<String> columnList,
			int order) {
