...
public class LuceneQuery extends QueryImpl implements Query {
	 ...
	 *            the jpa query
	 */
	public LuceneQuery(EntityManagerImpl em, MetadataManager metadataManager,
			String jpaQuery) {
	...
	public List<?> getResultList() {
		...
		Map<String, String> searchFilter = getEntityManager().getIndexManager()
				.search(q, -1, maxResult);
		if (isAliasOnly()) {
			return getEntityManager().find(getEntityClass(),
					new HashSet(searchFilter.values()).toArray());
		} else {
