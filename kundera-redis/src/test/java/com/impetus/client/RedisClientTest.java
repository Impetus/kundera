...
public class RedisClientTest {
	...
	private static Logger logger = LoggerFactory
			.getLogger(RedisClientTest.class);

	@Rule
	public ContiPerfRule i = new ContiPerfRule(new ReportModule[] {
			new CSVSummaryReportModule(), new HtmlReportModule() });
	...
	public void testCRUDWithBatch() {
		...
		for (int i = 0; i < 9; i++) {
			...
			if (i >= 5) {
				PersonRedis result = (PersonRedis) client.find(
						PersonRedis.class, ROW_KEY + i);
				...
				Assert.assertEquals(result.getPersonName(),
	...
	public void testPersistJoinTableData() {
		...
		JoinTableData joinTableData = new JoinTableData(OPERATION.INSERT,
				"redis", "redisjointable", "joincolumn",
		...
		List<String> columns = client.getColumnsById(schemaName, tableName,
				joinColumn, inverseJoinColumn, joinKey);
		...
		client.deleteByColumn(schemaName, tableName, inverseJoinColumn,
				inverseJoinKey1);
		client.deleteByColumn(schemaName, tableName, inverseJoinColumn,
				inverseJoinKey2);

		columns = client.getColumnsById(schemaName, tableName, joinColumn,
				inverseJoinColumn, joinKey);
