...
public class SchemaConfiguration implements Configuration {
	...
	public void configure() {
		...
		// if association exists, compute it and if required to create table
		// info for different pu(passing association).
		// then create and add it to map
		...
		for (String persistenceUnit : persistenceUnits) {
			if (getKunderaProperty(persistenceUnit) != null) {
				...
				for (EntityMetadata entityMetadata : entityMetadataMap.values()) {
					...
					tableInfo.setTableIdType(entityMetadata.getIdColumn()
							.getField().getType().toString());
					tableInfo.setAction(SchemaAction
							.instanceOf(getKunderaProperty(persistenceUnit)));
					...
					if (!tableInfos.isEmpty() && tableInfos.contains(tableInfo)) {
						...
						if (tableInfo.getType().equalsIgnoreCase("Standard")) {
							for (Column column : entityMetadata
									.getColumnsAsList()) {
								if (!tableInfo.getColumnMetadatas().contains(
										getColumn(column))) {
									tableInfo.getColumnMetadatas().add(
											getColumn(column));
								}
							}
						} else {
							List<EmbeddedColumnInfo> embeddedColumnInfos = new ArrayList<EmbeddedColumnInfo>();
							for (EmbeddedColumn embeddedColumn : entityMetadata
									.getEmbeddedColumnsAsList()) {
								embeddedColumnInfos
										.add(getEmbeddedColumn(embeddedColumn));
								tableInfo
										.setEmbeddedColumnMetadatas(embeddedColumnInfos);
							}
						}
					} else {
						if (tableInfo.getType().equalsIgnoreCase("Standard")) {
					...
					for (Relation relation : relations) {
						...
						if ((relationType.equals(ForeignKey.ONE_TO_MANY) && relation
								.getJoinColumnName() != null)
							...
							if (targetEntityMetadata.equals(entityMetadata)) {
								if (tableInfo.getColumnMetadatas() == null) {
									...
									columnMetadatas.add(getJoinColumn(relation
											.getJoinColumnName()));
									tableInfo
											.setColumnMetadatas(columnMetadatas);
								} else if (tableInfo.getColumnMetadatas()
										.contains(
												getJoinColumn(relation
														.getJoinColumnName()))) {
									tableInfo.getColumnMetadatas().add(
											getJoinColumn(relation
													.getJoinColumnName()));
								...
								String pu = targetEntityMetadata
										.getPersistenceUnit();
								Type targetEntityType = targetEntityMetadata
										.getType();
								TableInfo targetTableInfo = new TableInfo();
								targetTableInfo
										.setTableName(targetEntityMetadata
												.getTableName());
								targetTableInfo
										.setIndexable(targetEntityMetadata
												.isIndexable());
								...
								targetTableInfo
										.setTableIdType(targetEntityMetadata
												.getIdColumn().getField()
												.getType().toString());
								targetTableInfo
										.setAction(SchemaAction
												.instanceOf(getKunderaProperty(persistenceUnit)));
								if (!pu.equals(persistenceUnit)) {
									List<TableInfo> targetTableInfos = puToSchemaCol
											.get(pu);
									...
									if (!targetTableInfos.isEmpty()
											&& targetTableInfos
													.contains(targetTableInfo)) {
										int idx = targetTableInfos
												.indexOf(targetTableInfo);
										targetTableInfo = targetTableInfos
												.get(idx);

										Column column = entityMetadata
												.getIdColumn();
										targetTableInfo.getColumnMetadatas()
												.add(getColumn(column));
										targetTableInfos.add(targetTableInfo);
									} else {
										List<ColumnInfo> columnInfos = new ArrayList<ColumnInfo>();
										columnInfos
												.add(getColumn(entityMetadata
														.getIdColumn()));
										targetTableInfo
												.setColumnMetadatas(columnInfos);
										targetTableInfos.add(targetTableInfo);
									}
									...
									if (!tableInfos.isEmpty()
										...
										Column column = entityMetadata
												.getIdColumn();
										if (!targetTableInfo
												.getColumnMetadatas().contains(
														getColumn(column))) {
											targetTableInfo
													.getColumnMetadatas().add(
															getColumn(column));
										}
							...
							} else if (!tableInfo.getColumnMetadatas().contains(getJoinColumn(relation.getJoinColumnName()))) {
								tableInfo.getColumnMetadatas().add(
										getJoinColumn(relation
												.getJoinColumnName()));
							}
	...
	private String getKunderaProperty(String persistenceUnit) {
		String KUNDERA_DDL_AUTO_PREPARE = KunderaMetadata.INSTANCE
				.getApplicationMetadata()
