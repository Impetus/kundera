	...
	public void testPersist() {
		...
		try {
			...
			List<RedisGeneratedIdStrategySequence> list = em.createQuery(
					"Select c from RedisGeneratedIdStrategySequence c")
					.getResultList();
			...
			em.clear();
			strategySequence = em.find(RedisGeneratedIdStrategySequence.class,
					id);
		...
		try {
			...
			List<RedisGeneratedIdWithOutSequenceGenerator> list = em
					.createQuery(
							"Select c from RedisGeneratedIdWithOutSequenceGenerator c")
					.getResultList();
			Assert.assertNotNull(list);
			Assert.assertEquals(1, list.size());
			...
			Object id = list.get(0).getId();
			em.clear();
			withOutSequenceGenerator = em.find(
					RedisGeneratedIdWithOutSequenceGenerator.class, id);
			Assert.assertNotNull(withOutSequenceGenerator);
			Assert.assertEquals("Kuldeep Kumar",
					withOutSequenceGenerator.getName());
		} catch (KunderaException e) {
			Assert.fail();
		}
		...
		try {
			...
			List<RedisGeneratedIdWithSequenceGenerator> list = em.createQuery(
					"Select c from RedisGeneratedIdWithSequenceGenerator c")
					.getResultList();
			Assert.assertNotNull(list);
			Assert.assertEquals(1, list.size());
			...
			Object id = list.get(0).getId();
			em.clear();
			withSequenceGenerator = em.find(
					RedisGeneratedIdWithSequenceGenerator.class, id);
			Assert.assertNotNull(withSequenceGenerator);
			Assert.assertEquals("Kuldeep Kumar Mishra",
					withSequenceGenerator.getName());
		} catch (KunderaException e) {
			Assert.fail();
		}
