...
public class EntityManagerImpl implements KunderaEntityManager {
	 ...
	 * @return the e
	 */
	protected <E> E immediateLoadAndCache(Class<E> entityClass,
			Object primaryKey) {
		try {
	...
	public final void remove(Object e) {
		...
		try {
			...
			List<EnhancedEntity> reachableEntities = entityResolver.resolve(e,
					CascadeType.REMOVE, this.client.getType());
	...
	public final <E> E merge(E e) {
		...
		try {
			...
			List<EnhancedEntity> reachableEntities = entityResolver.resolve(e,
					CascadeType.MERGE, this.client.getType());
			...
			for (EnhancedEntity o : reachableEntities) {
				...
				eventDispatcher
						.fireEventListeners(metadata, o, PreUpdate.class);
				client.writeData(this, o, metadata);

				getIndexManager().update(metadata, o.getEntity());

				// fire PreUpdate events
				eventDispatcher.fireEventListeners(metadata, o,
						PostUpdate.class);
			}
		} catch (Exception exp) {
			throw new PersistenceException(exp);
	...
	public final void persist(Object e) {
		...
		try {
			...
			List<EnhancedEntity> reachableEntities = entityResolver.resolve(e,
					CascadeType.PERSIST, this.client.getType());
	...
	public void persistData(EnhancedEntity e) {
		try {
			EntityMetadata metadata = getMetadataManager().getEntityMetadata(
			...
			log.error("Error while persisting entity into datastore, Details: "
					+ ex.getMessage());
			throw ex;
		} catch (Exception ex) {
			ex.printStackTrace();
