...
public class PelopsDataHandler {
	...
	public <E> E fromThriftRow(Selector selector, EntityManagerImpl em,
			Class<E> clazz, EntityMetadata m, String rowKey) throws Exception {
		List<String> superColumnNames = m.getEmbeddedColumnFieldNames();
		E e = null;
		if (!superColumnNames.isEmpty()) {
			List<SuperColumn> thriftSuperColumns = selector
					.getSuperColumnsFromRow(m.getTableName(), rowKey,
							Selector.newColumnsPredicateAll(true, 10000),
							ConsistencyLevel.ONE);
			e = fromSuperColumnThriftRow(em, clazz, m,
					new PelopsClient().new ThriftRow(rowKey, m.getTableName(),
							null, thriftSuperColumns));

		} else {
			List<Column> columns = selector.getColumnsFromRow(m.getTableName(),
					new Bytes(rowKey.getBytes()),
					Selector.newColumnsPredicateAll(true, 10),
					ConsistencyLevel.ONE);

			e = fromColumnThriftRow(em, clazz, m,
					new PelopsClient().new ThriftRow(rowKey, m.getTableName(),
							columns, null));
	...
	public <E> List<E> fromThriftRow(Selector selector, EntityManagerImpl em,
			Class<E> clazz, EntityMetadata m, String... rowIds)
			throws Exception {
	 ...
	 *             the exception
	 */
	public <E> E fromColumnThriftRow(EntityManagerImpl em, Class<E> clazz,
			EntityMetadata m, PelopsClient.ThriftRow thriftRow)
			throws Exception {
		...
		PropertyAccessorHelper.set(e, m.getIdColumn().getField(),
				thriftRow.getId());
		...
		for (Column c : thriftRow.getColumns()) {
			...
			com.impetus.kundera.metadata.model.Column column = m
					.getColumn(name);
			if (null == column) {
				// it could be some relational column
				populateRelationshipEntities(em, thriftRow, e,
						m.getRelation(name), value);
	 ...
	 * objects
	 */
	public <E> E fromSuperColumnThriftRow(EntityManagerImpl em, Class<E> clazz,
			EntityMetadata m, PelopsClient.ThriftRow tr) throws Exception {
		...
		for (SuperColumn sc : tr.getSuperColumns()) {
			...
			if (scName.indexOf(Constants.EMBEDDED_COLUMN_NAME_DELIMITER) != -1) {
				scNamePrefix = MetadataUtils
						.getEmbeddedCollectionPrefix(scName);
				embeddedCollectionField = superColumnNameToFieldMap
						.get(scNamePrefix);

				if (embeddedCollection == null) {
					embeddedCollection = MetadataUtils
				...
				for (Column column : sc.getColumns()) {
					...
					if (intoRelations) {
						populateRelationshipEntities(em, tr, e,
								m.getRelation(name), value);
					} else {
						Field columnField = columnNameToFieldMap.get(name);
						PropertyAccessorHelper.set(embeddedObject, columnField,
								value);
					}
				...
				ElementCollectionCacheManager.getInstance()
						.addElementCollectionCacheMapping(tr.getId(),
				...
				if (intoRelations) {
					...
					Field superColumnField = superColumnNameToFieldMap
							.get(scName);
					...
					for (Column column : sc.getColumns()) {
						...
						Field columnField = columnNameToFieldMap.get(name);
						PropertyAccessorHelper.set(superColumnObj, columnField,
								value);
					}
					PropertyAccessorHelper.set(e, superColumnField,
							superColumnObj);
		...
		if (embeddedCollection != null && !embeddedCollection.isEmpty()) {
			PropertyAccessorHelper.set(e, embeddedCollectionField,
					embeddedCollection);
		}
		return e;
	}

	public Object populateEmbeddedObject(SuperColumn sc, EntityMetadata m)
			throws Exception {
		Field embeddedCollectionField = null;
	 ...
	 *             the exception
	 */
	public PelopsClient.ThriftRow toThriftRow(PelopsClient client,
			EnhancedEntity e, EntityMetadata m, String columnFamily)
			throws Exception {
	...
	private void addColumnsToThriftRow(long timestamp,
			PelopsClient.ThriftRow tr, EntityMetadata m, EnhancedEntity e)
			throws Exception {
		List<Column> columns = new ArrayList<Column>();

		// Iterate through each column-meta and populate that with field values
		for (com.impetus.kundera.metadata.model.Column column : m
				.getColumnsAsList()) {
	...
	private void addSuperColumnsToThriftRow(long timestamp,
			PelopsClient client, PelopsClient.ThriftRow tr, EntityMetadata m,
		...
		for (EmbeddedColumn superColumn : m.getEmbeddedColumnsAsList()) {
			...
			Object superColumnObject = PropertyAccessorHelper.getObject(
					e.getEntity(), superColumnField);
			...
			if (superColumnObject instanceof Collection) {
				...
				if (ecCacheHandler.isCacheEmpty()) { // First time insert
					...
					for (Object obj : (Collection) superColumnObject) {
						superColumnName = superColumn.getName()
								+ Constants.EMBEDDED_COLUMN_NAME_DELIMITER
								+ count;
						SuperColumn thriftSuperColumn = buildThriftSuperColumn(
								superColumnName, timestamp, superColumn, obj);
					...
					for (Object obj : (Collection) superColumnObject) {
						superColumnName = ecCacheHandler
				...
				superColumnName = superColumn.getName();
				SuperColumn thriftSuperColumn = buildThriftSuperColumn(
						superColumnName, timestamp, superColumn,
						superColumnObject);
				tr.addSuperColumn(thriftSuperColumn);
			}
	 ...
	 */
	public void addRelationshipsToThriftRow(long timestamp,
			PelopsClient.ThriftRow tr, EnhancedEntity e, EntityMetadata m)
			throws PropertyAccessException {
		if (!m.getEmbeddedColumnsAsList().isEmpty()) {
			...
			if (!columns.isEmpty()) {
				...
				superCol.setName(PropertyAccessorFactory.STRING
						.toBytes(Constants.FOREIGN_KEY_EMBEDDED_COLUMN_NAME));
	...
	private SuperColumn buildThriftSuperColumn(String superColumnName,
			long timestamp, EmbeddedColumn superColumn, Object superColumnObject)
			throws PropertyAccessException {
		List<Column> thriftColumns = new ArrayList<Column>();
		for (com.impetus.kundera.metadata.model.Column column : superColumn
				.getColumns()) {
			Field field = column.getField();
			String name = column.getName();

			try {
				byte[] value = PropertyAccessorHelper.get(superColumnObject,
						field);
				if (null != value) {
					Column thriftColumn = new Column();
					thriftColumn.setName(PropertyAccessorFactory.STRING
							.toBytes(name));
		...
		thriftSuperColumn.setName(PropertyAccessorFactory.STRING
				.toBytes(superColumnName));
	...
	public <E> void populateRelationshipEntities(EntityManagerImpl em,
			PelopsClient.ThriftRow tr, E e, Relation relation, byte[] value)
			throws PropertyAccessException {
