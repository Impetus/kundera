	...
	protected void tryOperation(String[] ALL_PUs_UNDER_TEST) {
		try {
			...
			for (int i = 0; i < ALL_PUs_UNDER_TEST.length; i++) {
				metaModel = KunderaMetadata.INSTANCE.getApplicationMetadata()
						.getMetamodel(ALL_PUs_UNDER_TEST[i]);

				for (int i1 = 0; i1 < ALL_PUs_UNDER_TEST.length; i1++) {
					if (i != i1) {
						Map<Class<?>, EntityType<?>> original = getManagedTypes((MetamodelImpl) metaModel);

						Metamodel m = KunderaMetadata.INSTANCE
...
//				em.clear();
	...
	private Map<Class<?>, EntityType<?>> getManagedTypes(MetamodelImpl metaModel) {
		try {
			...
			if (metaModel != null)
				managedTypesFields = metaModel.getClass().getDeclaredField(
						"entityTypes");
			if (managedTypesFields != null
					&& !managedTypesFields.isAccessible()) {
				managedTypesFields.setAccessible(true);

				return ((Map<Class<?>, EntityType<?>>) managedTypesFields
						.get(metaModel));
			}
