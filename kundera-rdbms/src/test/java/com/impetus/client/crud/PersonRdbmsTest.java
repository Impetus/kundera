...
public class PersonRdbmsTest extends BaseTest {
	...
	public void onInsertRdbms() throws Exception {
		...
		findQuery = em
				.createQuery("Select p from PersonRDBMS p where p.personName = vivek");
		allPersons = findQuery.getResultList();
		Assert.assertNotNull(allPersons);
		Assert.assertTrue(allPersons.isEmpty());

		findQuery = em
				.createQuery("Select p.age from PersonRDBMS p where p.personName = vivek");
		allPersons = findQuery.getResultList();
		Assert.assertNotNull(allPersons);
		Assert.assertTrue(allPersons.isEmpty());
		...
		assertFindByNameAndAgeGTAndLT(em, "PersonRDBMS", PersonRDBMS.class,
				"vivek", "10", "20", "personName");
		assertFindByNameAndAgeBetween(em, "PersonRDBMS", PersonRDBMS.class,
				"vivek", "10", "15", "personName");
		assertFindByRange(em, "PersonRDBMS", PersonRDBMS.class, "1", "2",
				"personId");
	}
	...
	public void onMergeRdbms() {
		...
		assertOnMerge(em, "PersonRDBMS", PersonRDBMS.class, "vivek",
				"newvivek", "personName");
